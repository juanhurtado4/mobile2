...
class Users(Resource):
    ...
    def post(self):
        ...
        result = users_collection.insert(new_user)
...
class Trip(Resource):
    ...
    def post(self):
        ...
        # result = self.trip_collection.insert(new_trip)
        trip = trip_collection.find_one({"_id": result.inserted_id})
        # trip = self.trip_collection.find_one({"_id": result.inserted_id})
        return trip

    def get(self):
        trip_name = request.args.get('destination')
        trip_collection = app.db.trips
        # trip = self.trip_collection.find_one({'trip_name': trip_name})
        trip = trip_collection.find_one({'destination': trip_name})
        pdb.set_trace()        
        return trip
